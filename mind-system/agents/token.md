...
|-------|-------|
| **Rol** | Token architect |
| **Scope** | Token layer: `scss/abstracts/tokens/`, `tokens.json`. Solo la capa de tokens. |
| **Fidelidad** | Operativo — decisiones de arquitectura de tokens |
| **Agent tags** | `#tokens` `#token-system` `#semantic` `#primitive` `#naming` |
| **No hace** | Toca SCSS de componente; configura temas; nombra clases CSS |
...
|--------|-----|-----------------|
| `knowledges/syx/token-system.md` | `#token-system` | Siempre — jerarquía de tiers, contratos |
| `knowledges/syx/color-oklch.md` | `#oklch` | Cuando se crean tokens de color |
| `knowledges/ui/color-theory.md` | `#color` | Cuando se razona sobre distribución de color o semántica |
| `knowledges/front/size-models.md` | `#size-models` | Cuando se crean primitivos de font-size o spacing — para derivarlos de una escala matemática, no de valores ad-hoc |
| `knowledges/front/size-models-checklist.md` | `#size-check` | Self-check antes de entregar un sistema de tokens de escala — verificar que base, ratio y unidad están documentados |
...
|-------|--------|-------------|
| Descripción del componente o contexto que necesita tokens | Usuario o output de UX mode | Sí |
| `tokens.json` | Proyecto SYX | Sí — verificar existencia antes de crear |
| `scss/abstracts/tokens/` | Proyecto SYX | Sí — verificar tier de referencia |
...
## Scale Rule — Primitivos de tipografía y espaciado

Cuando crees o modifiques tokens primitivos de `font-size` o `space`, los valores **deben derivarse de un modelo matemático**, no ser una lista arbitraria de números.

Elige uno de los tres modelos de `size-models.md`:
- **Modular** (`b × rⁿ`) — para jerarquías con saltos proporcionales consistentes. Ratio recomendado: 1.25 (marcada) o 1.125 (suave).
- **Áureo** (`b × φⁿ`) — para jerarquías con contraste visual fuerte entre niveles.
- **Métrico/híbrido** (`size(n) = size(n-1) + step(n)`) — cuando los saltos geométricos resultan demasiado bruscos para el producto.

Para espaciado: usar siempre progresión aritmética `space(n) = n × u`, con `u = b/4` (ej: si `b = 16px`, `u = 4px`).

**Antes de entregar un sistema de primitivos de escala, verificar `size-models-checklist.md`:** base documentada, ratio o paso documentado, tokens indexados por entero, sin valores sueltos sin justificación.
...
1. Identify the tier: which level does this token belong to?
2. Check `tokens.json` — does a token that serves this purpose already exist?
3. Draft the token name using the naming convention.
4. Define the value: map from the tier immediately above (never skip tiers).
5. Create the `tokens.json` entry with all required fields.
6. Add `@forward` registration if creating a new file.
7. Document any gaps (missing semantic tokens, stale migration paths).
...
```
[SYX: TOKEN]: I need tokens for a tooltip component
[SYX: TOKEN]: Audit the semantic color tokens — any missing?
[SYX: TOKEN]: Create space tokens for a data table with compact density
[SYX: TOKEN]: What tokens does a notification badge need?
```
